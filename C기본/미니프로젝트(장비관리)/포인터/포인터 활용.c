...
int check_list = 0; //로그인 반환 값 전역변수로 적용 후 환영문자 변수로 사용 
...
int num_check_tool = 0; //장비 수 확인용 (삭제 포함) len
        ...
        case 1:
            while (1)
            {
                ...
                case 2:
                    ...
                    while (1)
                    {
                        printf("찾을 회원의 id을 입력해주세요(숫자 입력) : ");
                        scanf("%d", &num);
...
int tool_agree1(int right) {
    ...
    printf("분류   모델명     시리얼넘버   승인여부    입고일     점검내용 및 결과\n");
    printf("=============================================================================\n");
    for (i = 0; i < num_check_tool; i++) {
...
int tool_agree2(int right) {
    ...
    for (i = 0; i < num_check_tool; i++) {
        if (tool[i].toolnum == list[right].right - 20) {
            if (tool[i].tool_del == 유지) printf("%s     %s        %s        %s   %d-%d-%d\n", tool[i].toolname, tool[i].model, tool[i].SN, tool[i].agree, tool[i].year, tool[i].month, tool[i].day);
            for (j = 1; j < 3; j++) {   //3은 임시 점검갯수
                printf("%d. %s : ", j, tool[i].checklist[j]);
                if (tool[i].state[j] == 0) printf("미점검\n");
                else if (tool[i].state[j] == 1) printf("양호\n");
...
struct tool_info* createNode() 
{
    ...
    for (i = num_check_tool; i < num_check_tool + 1; i++)
    {
        ...
        if (check == 1)
        {
            temp->toolnum = make.toolnum; // 장비 분류 숫자 
            strcpy(temp->toolname, make.toolname); //장비 분류 문자
            strcpy(temp->model, make.model); //모델명 
            strcpy(temp->SN, make.SN); // 시리얼 넘버 
            temp->year = make.year;
