	...
	for (i = 0; i < num_check_tool; i++) {
		if (tool[i].toolnum == right - 20) {
			...
			for (j = 1; j < 3; j++) {   //3은 임시 점검갯수
				printf("%d. %s : ", j, ptool[i]->checklist[j]);  // tool[i].checklist[j]
