...
struct user_info list[num_user] = { {"system", "관리자", 22010001, 관리자, 유지, &list[1] } };
struct tool_info tool[num_tool];
struct user_info* plist[num_user] = { &list[0] };
struct tool_info* ptool[num_tool];
struct tool_info* head = &tool[0];

struct user_info* uhead = &list[0]; //user_info head 선언
...
int make_user() {
    ...
    for (;;) {
        ...
        if (check == 1) {
            list[num_check_user] = make;
            list[num_check_user].pu = &list[num_check_user + 1];
            list[num_check_user + 1].pu = NULL;
            plist[num_check_user] = &list[num_check_user];
            //printf("%s", plist[num_check_user]->name);
...
int member_search(void)  //조회
{
            ...
            while (1)
            {
                ...
                case 1:
                    ...
                    while (1)
                    {
                        ...
                        for (i = 0; i < num_check_user; i++)
                        {
                            if (strcmp(b, list[i].id) == 0)
                            {

                                printf("%10d%15s%15s           ", list[i].id, list[i].pw, list[i].name);

                                print_right(list[i].right);
 ...
 /*           for (i = 0; i < num_check_user; i++)
            ...
            case 1:
                ...
                for (i = 0; i < num_check_user; i++)
                {
                    for (j = 0; j < num_check_user - 1 - i; j++)
                    {
                ...
                for (i = 0; i < num_check_user; i++)
                {
                    if (plist[i]->user_del == 유지)
                    {
                        count++;
                        printf("%10d%15s%15s           ", plist[i]->id, plist[i]->pw, plist[i]->name);
                        print_right(plist[i]->right); 
                    }
                }
...
int toolmenu1()
{
    ...
    for (i = num_check_tool; i < num_check_tool + 1; i++)
    {
        ...
        if (check == 1)
        {
            ...
            /*tool[num_check_tool] = make;*/
            tool[i].num = ++i;
            //reset[num_check_tool] = &tool[num_check_tool]; //배열 초기화
...
int print_userall(void)
{
    ...
    struct user_info* temp = uhead;

    while ((temp->pu) != NULL)
    {
        count++;
        printf("%10d%15s%15s           ", temp->id, temp->pw, temp->name);
...
int search_node()
{
    ...
    case 1:
        ...
        printf("분류\t모델명\t시리얼넘버\t승인여부\t입고일\n");
        printf("===================================================================\n");
        while ((temp->next) != NULL) //head파일의 데이터부터 시작하여
        {
