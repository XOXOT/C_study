...
void menu_6() //6. 전체 학생의 전체 저장된 값을 출력
{
	...
	for (i = 0; i < len; i++)
	{
		printf("[ %d ]번 학생 이름:%s  국어:%d  영어:%d  수학:%d 과학:%d  총점:%d  평균:%f \n",
...
void menu_7() //7. 전체 학생의 전체 저장된 값을 성적순으로 출력
{
	...
	for (i = 0; i < len; i++)
	{
		printf("[ %d ]등 이름:%s  국어:%d  영어:%d  수학:%d  과학:%d 총점:%d 평균:%f \n",
...
void menu_8() //8. 저장된 학생의 데이터를 수정하는 기능
{
	...
	scanf("%s", name);
	printf("수정할 과목(kor, eng, math, sci 중 입력):");
	scanf("%s", subject);
	...
	for (i = 0; i < len; i++)
	{
		if (!strcmp(name, std[i].name))
		{
			...
			std[i].sum = (int)(std[i].kor + std[i].eng + std[i].math); //총점 변경
			std[i].average = (double)(std[i].kor + std[i].eng + std[i].math + std[i].sci) / 4; //평균 변경
		}
	}
	printf("변경이 완료되었습니다.\n");
	printf("\n\n");
...
void processMenu(char choice)
{
	switch (choice)
	{
	case '1': //1. 정보(이름, 국어, 영어, 수학, 과학)를 입력받아 평균을 구해 저장
		menu_1();
		break;
	...
	case '4': //4. 이름을 입력 받으면 국어, 영어, 수학, 과학, 총점, 평균을 출력
		menu_4();
		break;
